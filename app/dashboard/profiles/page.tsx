...
export default async function ProfilesPage({
  ...
  return (
    <div>
      <PageHeader title="Profiles" description="Search a user to view their full Heaven Land profile." />
      ...
      {q && !result && <UnavailableNotice label="Profiles" />}
      ...
      {result && (
        <div className={`${cardClass} space-y-2`}>
          ...
          {result.items.map((u) => (
            ...
            >
              {u.username} <span className="text-hl-faint">· Level {u.level}</span>
            </Link>
          ))}
        </div>
      )}
